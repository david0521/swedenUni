    ...
    methods: {
            ...
            try {
                 ...
                 */
                 ...
                 if (!this.uniName && !this.cityName) {
                    response = await axios.get('https://51.13.63.57/api/universities')
                 }

                 else if ((this.uniName && !this.cityName) || (this.uniName && this.cityName)) {
                    const encodedUniName = encodeURIComponent(this.uniName);
                    response = await axios.get(`https://51.13.63.57/api/universities/name/${encodedUniName}`)

                 } else {
                    response = await axios.get(`https://51.13.63.57/api/universities/byCity?city=${this.cityName}`)
                 }
            ...
            try {
                ...
                const requests = universityNames.map(async (uniName) => {
                    try {
                        ...
                        const response = await axios.get(`https://51.13.63.57/api/universities/prospective/university?university=${encodedUniName}`);
        ...
        async checkLiked() {
            try {
                ...
                const response = await axios.get(`https://51.13.63.57/api/users/${userId}/prospective/universities`);
        ...
        async like(index) {
            try {
                ...
                if (currentLikeStatus) {
                    response = axios.delete(`https://51.13.63.57/api/users/${userId}/universities`, 
                        {
                            data: {
                    ...
                    response = axios.post(`https://51.13.63.57/api/users/${userId}/universities`, {
                        universities: universityId
                    })

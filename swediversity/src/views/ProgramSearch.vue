        ...
        async search() {
            try 
            {
                 ...
                 if (!this.programName && !this.programType && !this.meritPoint && !this.fee && this.selectedPrerequisites.length === 0) {
                    response = await axios.get('http://51.13.63.57/api/programs')
                 } else if (!this.programType && !this.meritPoint && !this.fee && this.selectedPrerequisites.length === 0) {
                    response = await axios.get(`http://51.13.63.57/api/programs/name/${this.programName}`)
                 }
                 ...
                 else 
                 {
                    ...
                    response = await axios.get(`http://51.13.63.57/api/programs/search?${params.toString()}`);
                 }
        ...
        async like(index) {
            try {
                ...
                if (currentLikeStatus) {
                    response = axios.delete(`http://51.13.63.57/api/users/${userId}/programs`, 
                        {
                            data: {
                                programs: programId
                            }
                    ...
                    response = axios.post(`http://51.13.63.57/api/users/${userId}/programs`, {
                        programs: programId
                    })

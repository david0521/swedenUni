  ...
  methods: {
    ...
    async postContent() {
      try {
        this.message = null;
        const userId = localStorage.getItem('userId');
        const response = await axios.post(`https://51.13.63.57/api/posts/contentType/programReview/userId/${userId}`,
          {
    ...
    async getContents() {
      try {
        const userId = localStorage.getItem('userId');
        const response = await axios.get(`https://51.13.63.57/api/posts/userId/${userId}?type=ProgramPost`)

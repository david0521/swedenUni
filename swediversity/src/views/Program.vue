<template>
  ...
  export default {
    ...
    methods: {
      ...
      async getProgram() {
        try {
            const response = await axios.get(`http://51.13.63.57/api/programs/name/${this.programName}`);
      ...
      async getMinMeritPoint() {
        try {
          const response = await axios.get(`http://51.13.63.57/api/records/name/${this.programName}/avg`)
